...
-- write a SQL query to list the employeesâ€™ name, increased their salary by 15%, and expressed as number of Dollars.
SELECT employees, to_char(15*salary,'$99,999') AS 'Salary' FROM employees;


-- write a SQL query to list the employees’ name, increased their salary by 15%, and expressed as number of Dollars.
SELECT * FROM new_project.employees;
SELECT employees_name , to_char(1.15*salary,'$99,999') AS 'Salary' FROM employees;
SELECT concat(Employee,Job) AS "Employee & Job" FROM employees;
...
SELECT EMPLOYEE_ID, FIRST_NAME,LAST_NAME, SALARY , to_date (HIRE_DATE,'MONTH DD,YYYY') FROM employees ;s
